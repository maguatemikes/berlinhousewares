...
async function loadCriticalData({context, params, request}: Route.LoaderArgs) {
  ...
  // Build the native Storefront filter input from the URL (same scheme as /collections/all).
  const filters: Array<Record<string, unknown>> = [];
  for (const f of searchParams.getAll('filter')) {
    try {
      filters.push(JSON.parse(f) as Record<string, unknown>);
    } catch {
  ...
  let sortKey: 'COLLECTION_DEFAULT' | 'PRICE' = 'COLLECTION_DEFAULT';
  let reverse = false;
  if (sort === 'price-asc') {
    sortKey = 'PRICE';
    reverse = false;
                ...
                <div>
                  ...
                  <div className="mt-8 flex flex-col items-center gap-3 border-t border-black/10 pt-6">
                    <p className="text-sm text-muted">
                      ...
                      {nodes.length === 1 ? 'product' : 'products'}
                    </p>
